      ...
      <div className="grid md:grid-cols-3 gap-6 mt-3">
        {plans.map((p) => (
          <div key={p.priceMonthly} className={`rounded-2xl border bg-white p-6 shadow-sm ${p.highlight ? 'ring-2 ring-[#F9D83C] border-[#F9D83C]' : ''}`}>
            ...
            <div className="mt-4">
              <a href="/signin" className={`inline-flex items-center justify-center rounded-lg px-4 py-2 w-full ${p.highlight ? 'bg-[#F9D83C] text-black hover:bg-[#F9D83C]/90' : 'bg-black text-white hover:bg-black/90'}`}>
                {t(lang, 'subscribe')}
              </a>
            </div>
          </div>
