...
export default function UploadBox() {
  ...
  const onDragOver = useCallback((e: React.DragEvent<HTMLLabelElement>) => { e.preventDefault(); setIsDragging(true); }, []);
  ...
  const upload = useMutation({
    mutationFn: async (file: File) => {
      ...
      if (sign?.error || !sign.uploadUrl || !sign.publicUrl) throw new Error(sign?.error || "Signature invalide");
      const putRes = await fetch(sign.uploadUrl, { method: "PUT", headers: { "Content-Type": file.type }, body: file });
      if (!putRes.ok) { const txt = await putRes.text().catch(() => ""); throw new Error(`R2 PUT failed: ${putRes.status} ${putRes.statusText} ${txt}`); }
      return { imagePublicUrl: sign.publicUrl as string };
    },
    onMutate: () => { setImageUrl(null); setCleanedUrl(null); toast.loading("Upload en cours…", { id: "up" }); },
    onSuccess: (d) => { setImageUrl(d.imagePublicUrl); toast.success("Image uploadée ✅", { id: "up" }); },
  ...
  const removePeople = useMutation({
    mutationFn: async () => {
      ...
      if (!imageUrl && file) { const b64 = await fileToBase64(file); imageDataUrl = `data:${file.type};base64,${b64}`; }
    ...
    onSuccess: (url) => { setCleanedUrl(url); setImageUrl(url); toast.success("Personnes retirées ✅", { id: "rp" }); },
        ...
        <div className="rounded-2xl border bg-white p-6 shadow-sm">
          ...
          >
            ...
            <div className="text-sm text-neutral-600">
              {file ? (<span className="font-medium text-black">{file.name}</span>) : (<>Glisse ton image ou <span className="underline">clique pour parcourir</span></>)}
            </div>
          </label>
        ...
        <div className="rounded-2xl border bg-white p-6 shadow-sm">
          ...
          {!imageUrl ? (
            ...
            <>
              ...
              <img src={imageUrl} alt="upload" className="mt-4 w-full max-h-[420px] object-contain rounded-xl ring-1 ring-black/5" />
              {runwayStatus && (
