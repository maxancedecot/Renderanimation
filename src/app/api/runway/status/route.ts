...
export async function GET(req: NextRequest) {
  try {
    ...
    const raw = (data?.status || data?.state || "pending").toString().toLowerCase();
    const status = ["pending","running","succeeded","failed"].includes(raw)
      ? raw
      : (raw.includes("success") ? "succeeded" : (raw.includes("fail") ? "failed" : raw));
